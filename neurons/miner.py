...
class Miner(BaseMinerNeuron):
    ...
    def __init__(self, config=None):
        ...
        chunk_scorer = "gen7heur8"  # hardcoded for gen7heur8 release
        ...
        bt.logging.info("[init] POKER44_CHUNK_SCORER=gen7heur8 (hardcoded for gen7heur8 release)")
        bt.logging.info(
        ...
        self.model_manifest = build_local_model_manifest(
            ...
            defaults={
                "model_name": "poker44_gen7heur8",
                "model_version": "7.6",
                "framework": "python-heuristic-rebalanced",
                "license": "MIT",
                "repo_url": "https://github.com/tomkaba/poker44-miner-gen7heur8",
                "repo_commit": _git_commit,
                "notes": "Gen7heur5 with per-batch 50/50 rebalance.",
                "open_source": True,
    ...
    async def forward(self, synapse: DetectionSynapse) -> DetectionSynapse:
        ...
        # gen7heur8 scorer bypasses per-hand ML models entirely
        chunk_scorer = "gen7heur8"  # hardcoded for gen7heur8 release

        if chunk_scorer == "gen7heur8":
            scores, chunk_routes, rebalance_stats = score_chunks_gen7heur8(chunks)
            bt.logging.info(
                "[miner] gen7heur8 rebalance | "
                ...
                f"target_bots={rebalance_stats['target_bots']} "
                f"flips={rebalance_stats['flips']} "
                f"final_bots={rebalance_stats['final_bots']} "
                f"final_humans={rebalance_stats['final_humans']}"
            )
            for index, (chunk, mode, route, score) in enumerate(zip(chunks, chunk_modes, chunk_routes, scores)):
                bt.logging.debug(
